...
const EVENT_LABEL = "Molino Pop-Up · Saturday Late-Night";
const PICKUP_DATE_LABEL = "Saturday, June 6, 2026 · 9:30 PM – 12:30 AM";
...
PICKUP & TIMING
Pickup is Saturday, June 6, 2026 between 9:30 PM and 12:30 AM at the address shared by the chef after your order is confirmed. Pizzas and pasta are made to order from a wood-fired Neapolitan oven and are best eaten within 15 minutes of pickup. If you cannot make your selected window, please notify the chef as early as possible.

ORDERS, INVENTORY & REFUNDS
Quantities are limited and pre-orders are fulfilled in the order received. Once your order is started, it is non-refundable. No-shows forfeit their order without refund.
...
export const Route = createFileRoute("/molino-june-6")({
  head: () => {
    const title = "Molino — Saturday Night Pop-Up | Chef Moshe Fhima · June 6";
    const desc =
      "One-night Neapolitan pop-up by Chef Moshe Fhima. Saturday, June 6, 2026 — 9:30 PM to 12:30 AM. Pre-order Margherita & La Bianca pizzas, Fusilloni alla Vodka, and Nutella Calzone.";
    return {
      meta: [
        { title },
        ...
        { property: "og:url", content: "https://www.eatcolorfull.com/molino-june-6" },
      ],
    };
  },
  component: MolinoJune6Page,
});

const EVENT_SLUG = "molino-june-6";
...
// 12:00 AM rolls over to Sun Jun 7.
...
function PickupCountdown({ slot }: { slot: string }) {
  ...
  const fireAt = new Date(target.getTime() - 10 * 60 * 1000);
  ...
  return (
    ...
    >
      ...
      <p className="mt-2 opacity-80">
        🔥 Chef Moshe fires your order around <strong>{fireLabel}</strong> so it lands hot
...
function MolinoJune6Page() {
  ...
  const biancaAddonsCents = biancaMushrooms ? 300 : 0;
  ...
  const subtotalCents =
    margheritaLineCents + biancaLineCents + pastaLineCents + calzoneLineCents;
  ...
  function buildOrderSummary() {
    ...
    if (biancaQty > 0) {
      lines.push(
        `${biancaQty} × La Bianca Pizza${biancaMushrooms ? " (add-on: Wild Mushrooms)" : ""} — $${(biancaLineCents / 100).toFixed(2)}`,
      );
    }
    if (pastaQty > 0) {
      lines.push(
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const margAddonLabels = MARGHERITA_ADDONS.filter((a) =>
        ...
        .map((a) => a.label)
        .join(", ");
      const biancaAddonLabels = biancaMushrooms ? "Wild Mushrooms" : "";
      ...
      }).catch((e) => console.error("[molino-june-6] notify failed", e));
    ...
    <div style={{ backgroundColor: "#0d0d0d", color: "#e8d5b7" }} className="min-h-screen">
      <section className="px-6 pt-16 pb-12 text-center md:pt-24 md:pb-16">
        <div className="mx-auto max-w-3xl">
          ...
          <p className="mx-auto mt-6 max-w-md text-xs leading-relaxed tracking-[0.18em] opacity-70">
            DM TO PLACE ORDER / CHOOSE TIME SLOT. LIMITED QUANTITIES AVAILABLE.
          </p>

          <a
            href="#order"
            className="mt-10 inline-flex h-12 items-center px-8 text-[11px] uppercase tracking-[0.32em] transition-opacity hover:opacity-90"
        ...
        <div className="mx-auto max-w-2xl">
          ...
          <div className="mt-12 space-y-12">
            ...
            <MenuItem
              ...
              title="PASTA"
              price="$25"
              note="Fusilloni alla Vodka · organic, Di Gragnano IGP"
      ...
      <section id="order" className="scroll-mt-20 px-6 py-16 md:py-24" style={{ backgroundColor: "#0a0a0a" }}>
        <div className="mx-auto max-w-3xl">
          ...
          <p className="mx-auto mt-6 max-w-xl text-center text-sm leading-relaxed opacity-75">
            ...
            items, choose a pickup window, and Chef Moshe will confirm the pickup address
            by email.
          </p>
          ...
          >
            ...
            <div>
              ...
              <p className="mt-1 text-xs opacity-70">Organic · Di Gragnano IGP</p>
            ...
            <div>
              ...
              <p className="mt-1 text-xs opacity-70">Dessert</p>
            ...
            <div>
              ...
              <p className="mt-2 text-xs opacity-70">
                Pick the time you want to pick up — Chef Moshe will fire your order so
                it's pulled from the wood oven right before then, hot and ready when
                you arrive. Best eaten within 15 minutes.
              </p>
                ...
                {PICKUP_SLOTS.map((slot) => {
                  ...
                  return (
                    <button
                      ...
                      className="h-11 border text-[11px] tracking-wider transition-colors"
                      style={{
                        borderColor: active ? "#e8d5b7" : "#3a2f24",
                        backgroundColor: active ? "#e8d5b7" : "transparent",
                        color: active ? "#0d0d0d" : "#e8d5b7",
                      }}
                    >
                      {slot}
                    </button>
            ...
            <div className="border-t pt-5 text-sm" style={{ borderColor: "#3a2f24" }}>
              ...
              {pastaQty > 0 && (
                <div className="flex justify-between py-1">
                  <span className="opacity-80">
                    {pastaQty} × Fusilloni
                    {pastaAddonsCents > 0 ? ` (+$${(pastaAddonsCents / 100).toFixed(2)} ea)` : ""}
                  </span>
                  <span>${(pastaLineCents / 100).toFixed(2)}</span>
                </div>
              )}
              {calzoneQty > 0 && (
                <div className="flex justify-between py-1">
                  <span className="opacity-80">{calzoneQty} × Nutella Calzone</span>
                  <span>${(calzoneLineCents / 100).toFixed(2)}</span>
                </div>
              )}
